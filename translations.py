class Translations:
    convince_message = {
        'en': "I work better than our managers) Select your status and I will help you:",
        'ru': "Я работаю лучше чем наши менеджеры) Выберите ваш статус и я помогу вам:",
        'kz': "Менеджерлерімізден жақсы жұмыс істеймін) Статусыңызды таңдаңыз, мен сізге көмектесемін:"
    }

    confirm_contact_button = {
        'en': "Still contact the administrator!",
        'ru': "Всё равно связаться с администратором!",
        'kz': "Әкімшімен бәрібір байланысу!"
    }

    other_option_message = {
        "kz": "Жауабыңызды жіберіңіз және біз оны администраторға жібереміз.",
        "ru": "Отправьте ваш вопрос, и мы передадим его администратору.",
        "en": "Please send your question, and we will forward it to the administrator."
    }

    return_to_client_status_prompt = {
        'en': "Would you like to return to the Client Status Selection?",
        'ru': "Хотите вернуться к выбору статуса клиента?",
        'kz': "Клиент статусын таңдауға оралу?"
    }

    return_to_client_status_button = {
        'en': "Return to Client Status",
        'ru': "Вернуться к статусу участника",
        'kz': "Клиент статусына оралу"
    }

    contact_admin_prompt = {
        'en': "Would you like to contact an administrator?",
        'ru': "Хотите задать вопрос администратору?",
        'kz': "Администратормен хабарласу керек пе?"
    }

    contact_admin_button = {
        'en': "Contact administrator",
        'ru': "Связаться с администратором",
        'kz': "Администратормен хабарласу"
    }

    other = {
        "kz": "Басқа сұрақ",
        "ru": "Задать вопрос",
        "en": "Another question"
    }

    back = {
        "kz": "⬅️ Артқа",
        "ru": "⬅️ Назад",
        "en": "⬅️ Back"
    }

    nps = {
        "kz": "nps",
        "ru": "Проверить NPS",
        "en": "nps"
    }

    nps_message = {
        "kz": "type nps code",
        "ru": "Введите код NPS",
        "en": "type nps code"
    }


    admin_response = {
        "kz": "Администратордың жауабы",
        "ru": "Ответ от администратора",
        "en": "Admin's answer"
    }

    ask_question_prompt = {
        "kz": "Сұрауға жауап беру үшін жазыңыз:",
        "ru": "Напишите ваш вопрос для ответа:",
        "en": "Please write your question to get a response:"
    }

    variant_promt = {
        "kz": "Опцияны таңдаңыз:",
        "ru": "Выберите вариант:",
        "en": "Please select an option:"
    }

    status_message = {
        "kz": "no",
        "ru": "По какому из разделов у вас возникли вопросы?"
    }

    variant_selected = {
        "kz": "Сіз таңдадыңыз:",
        "ru": "Вы выбрали вариант:",
        "en": "You have chosen the option:"
    }

    language_prompt = {
        "kz": "Тілді таңдаңыз:",
        "ru": "Выберите язык:",
        "en": "Please select a language:"
    }

    language_selected = {
        "kz": "Сіз Қазақ тілін таңдадыңыз.",
        "ru": "Вы выбрали Русский язык.",
        "en": "You selected English language."
    }

    status_prompt = {
        "kz": "Бұл бот бүкіл оқу процесінде бағдарламаға қатысушылар үшін нұсқаулық ретінде қызмет етеді. \n\n<b>Статус таңдаңыз:</b>",
        "ru": "Привет! 👋 Я бот программы Tech Orda. Я буду служить вашим гидом в течение всего процесса обучения. \n\n<b>Давайте начнем, выберите ваш статус:</b>",
        "en": "This bot serves as a guide for program participants throughout the learning process. \n\n<b>Select your status:</b>"
    }

    status_not_started = {
        "kz": "Студенттерді оқытуды бастамадым",
        "ru": "Еще не начал обучать студентов",
        "en": "Hasn't started teaching students yet"
    }

    status_teaching = {
        "kz": "Студенттерді оқытып жатырмын",
        "ru": "Обучаю студентов",
        "en": "Teaching students"
    }

    status_finished = {
        "kz": "Студенттерді оқыту аяқталды",
        "ru": "Закончил обучать студентов",
        "en": "Finished teaching students"
    }

    status_selected_not_started = {
        "kz": "Сіз таңдадыңыз: Студенттерді оқытуды бастамадым.",
        "ru": "Вы выбрали статус: Еще не начал обучать студентов.",
        "en": "You selected status: Hasn't started teaching students yet."
    }

    status_selected_teaching = {
        "kz": "Сіз таңдадыңыз: Студенттерді оқытып жатырмын.",
        "ru": "Вы выбрали статус: Обучаю студентов.",
        "en": "You selected status: Teaching students."
    }

    status_selected_finished = {
        "kz": "Сіз таңдадыңыз: Студенттерді оқыту аяқталды.",
        "ru": "Вы выбрали статус: Закончил обучать студентов.",
        "en": "You selected status: Finished teaching students."
    }

    showcase_it_school = {
        "kz": "It мектептерінің көрмесі",
        "ru": "Витрина IT школ",
        "en": "IT School Showcase"
    }

    student_selection_process = {
        "kz": "Студенттерді 3 кезеңдік іріктеу",
        "ru": "3-х этапный отбор студентов",
        "en": "3-stage student selection process"
    }

    instructions_links = {
        "round_1": "https://drive.google.com/file/d/1tmJ62fl_jKmWw0qnBXOtlUJif6mTQ_Ld/view",
        "round_2": "https://drive.google.com/file/d/1HrKxblRtI93o4axY5x9Pzo2IcgoifYne/view",
        "round_3": "https://drive.google.com/file/d/19ju_pkzoPM6x7I09MU7860P1GG8clU2u/view"
    }

    student_selection_instructions = {
        "kz": (
            f"<a href=\"{instructions_links['round_1']}\">1-раунд студенттерді іріктеу бойынша нұсқаулық</a>\n"
            f"<a href=\"{instructions_links['round_2']}\">2-раунд студенттерді іріктеу бойынша нұсқаулық</a>\n"
            f"<a href=\"{instructions_links['round_3']}\">3-раунд студенттерді іріктеу бойынша нұсқаулық</a>"
        ),
        "ru": (
            f"<a href=\"{instructions_links['round_1']}\">Инструкция по 1 раунду отбора студентов</a>\n"
            f"<a href=\"{instructions_links['round_2']}\">Инструкция по 2 раунду отбора студентов</a>\n"
            f"<a href=\"{instructions_links['round_3']}\">Инструкция по 3 раунду отбора студентов</a>"
        ),
        "en": (
            f"<a href=\"{instructions_links['round_1']}\">Instructions for Round 1 of Student Selection</a>\n"
            f"<a href=\"{instructions_links['round_2']}\">Instructions for Round 2 of Student Selection</a>\n"
            f"<a href=\"{instructions_links['round_3']}\">Instructions for Round 3 of Student Selection</a>"
        )
    }

    student_contract_link = {
        "contract_1": "https://drive.google.com/file/d/1tmJ62fl_jKmWw0qnBXOtlUJif6mTQ_Ld/view"
    }

    student_contracts_document = {
        "kz": f"<a href=\"{student_contract_link['contract_1']}\">Шарт үлгісі</a>",
        "ru": f"<a href=\"{student_contract_link['contract_1']}\">Шаблон договора</a>",
        "en": f"<a href=\"{student_contract_link['contract_1']}\">Contract template</a>",
    }

    student_contracts = {
        "kz": "Студенттермен шарттар",
        "ru": "Договоры со студентами",
        "en": "Student contracts"
    }

    signing_contract_fund_school = {
        "kz": "Шартқа қол қою Фонд-школа",
        "ru": "Подписание договора Фонд-школа",
        "en": "Signing contract with the school fund"
    }

    advance_payment = {
        "kz": "Аванстық төлем",
        "ru": "Авансовый платеж",
        "en": "Advance payment"
    }

    monthly_report = {
        "kz": "Ай сайынғы есеп беру",
        "ru": "Ежемесячная отчетность",
        "en": "Monthly Report"
    }

    monthly_report_message = {
        "kz": "Бағдарламаға қатысушы ай сайын есепті кезеңнен кейінгі айдың 10 (оныншы) күніне дейін Бағдарлама шеңберінде курс (лар) бойынша оқу мерзімі ішінде порталда Шартқа сәйкес ақпаратты қамтитын есептің электрондық нысанын толтырады.  Бағдарламаға қатысушы нысанға қол қойғаннан кейін Бағдарламаға қатысушыдан оқитын әрбір Студент есепті айдан кейінгі тиісті айдың 20 (жиырмасыншы) күніне дейінгі мерзімде порталдағы есеп деректерінің дұрыстығын растайды.",
        "ru": "Участник Программы ежемесячно до 10 (десятого) числа месяца следующего за отчетным периодом, на протяжении срока обучения по Курсу(-ам) в рамках Программы, заполняет электронную форму отчета на Портале, содержащую информацию согласно Договору.  После подписания формы Участником Программы, каждый Студент, обучающийся у Участника Программы, в срок до 20 (двадцатого) числа соответствующего месяца, следующего за отчетным подтверждает корректность данных отчета на Портале.",
        "en": "No translate"
    }

    learning_change = {
        "kz": "Оқу аясындағы өзгерістер",
        "ru": "Изменения в рамках обучения",
        "en": "Learning Changes"
    }

    learning_change_message = {
        "kz": "Қорға қандай өзгерістер туралы хабарлау қажет: 1. Студентті ауыстыру немесе алып тастау 2. Студенттің дербес деректеріндегі өзгеріс 3. Компания деректемелерін өзгерту (заңды мекенжайын өзгерту, компания атауын өзгерту, банк деректемелері және басшыны ауыстыру) 4. Оқу кестесіндегі өзгерістер (оқу ұзақтығы, онлайн сабаққа сілтеме, сабақтарды өткізу күндері мен уақыты, курс бойынша оқытушы, оқуды аяқтау күні және Академия сағаттарының саны) 5. Сабақтарды өткізу орны (офлайн курстар үшін). Барлық өзгерістер порталда электрондық нысанда жүзеге асырылады astanahub.com.",
        "ru": "О каких изменениях необходимо оповещать Фонд: 1. Замена либо исключение студента 2. Изменение в персональных данных студента 3. Изменение реквизитов компании (Смена юридического адреса, смена названия компании, банковские реквизиты и смена руководителя) 4. Изменения в графике обучения (длительность обучения, ссылка на онлайн урок, дни и время проведения занятий, преподаватель по курсу, дата окончания обучения и количество академ часов) 5. Место проведения занятий (для оффлайн курсов). Все изменения осуществляются в электронной форме на портале astanahub.com.",
        "en": "No translate"
    }

    special_flow = {
        "kz": "Арнайы ағын",
        "ru": "Специализированный поток",
        "en": "Special Flow"
    }

    competence_assessment = {
        "kz": "Құзыреттілікті бағалау",
        "ru": "Оценка компетенций",
        "en": "Competence Assessment"
    }

    competence_assessment_message = {
        "kz": "Құзыреттілікті бағалау-Бағдарлама шеңберінде курстар бойынша оқыту тиімділігіне мониторинг жүргізу мақсатында Бағдарлама студенттерін (студенттерін) және(немесе) түлектерін бағалаудың кешенді әдісі.",
        "ru": "Оценка компетенции- комплексный метод оценки Студента(-ов) и(или) выпускников Программы с целью мониторинга эффективности обучения по Курсам в рамках Программы.",
        "en": "No trans"
    }

    financial_documents = {
        "kz": "Соңғы төлем",
        "ru": "Финальная оплата",
        "en": "Final payment"
    }

    final_payment_message = {
        "kz": "Әрбір студент үшін қаржыландыру сомасынан қалған төлем сомасы Бағдарламаға қатысушы мынадай шарттарды орындаған кезде бағдарламаға қатысушыға аударылады: 1. Біліктілікті бағалаудан өткен студент оқуды аяқтады. 2. Қорытынды есеп беру 3. Курстан өткені туралы сертификаттардың көшірмесін ұсыну 4. Қорға орындалған жұмыстардың бұрын жіберілген актісі және электрондық шот-фактура негізінде ресімделген тиісті түрде қалыптастырылған төлем шотын беру.",
        "ru": "Оставшаяся сумма Платежа от Суммы финансирования за каждого Студента перечисляется Участнику Программы, при выполнении Участником Программы следующих условий: 1. Завершения обучения Студентом который прошел оценку компетенции. 2. Предоставление финального отчета 3. Предоставления копии сертификатов о прохождении Курсов 4. Предоставления Фонду надлежащим образом сформированного счета на оплату, оформленного на основании ранее направленного акта выполненных работ и электронной счет-фактуры.",
        "en": "No t"
    }

    final_report = {
        "kz": "Қорытынды есеп",
        "ru": "Финальная отчетность",
        "en": "Final Report"
    }

    final_report_form = {
        "kz": "Қорытынды есеп формасын ашыңыз",
        "ru": "Открыть форму финального отчета",
        "en": "No t"
    }

    final_report_message = {
        "kz": "Оқу аяқталғаннан кейін Бағдарламаға қатысушы ай сайынғы қорытынды есептен басқа, Технопарк анықтаған нысан бойынша студенттің қорытынды есебі мен сертификатын ұсынады.",
        "ru": "По завершению обучения Участник Программы, в дополнение к заключительному ежемесячному отчету, предоставляет финальный отчет и сертификат Студента по форме, определенной Технопарком.",
        "en": "No t"
    }

    final_report_template_button = {
        "kz": "Қорытынды есепті толтыру үлгісі",
        "ru": "Шаблон заполнения финального отчета",
        "en": "No t"
    }

    final_report_template = {
        "kz": "Қорытынды есепті толтыру үлгісі",
        "ru": "Шаблон заполнения финального отчета",
        "en": "No t"
    }

    certificate_instructions_button = {
        "kz": "Сертификат үлгісіне арналған нұсқаулық",
        "ru": "Инструкция к шаблону сертификата",
        "en": "No t"
    }

    certificate_instructions_template = {
        "kz": "Сертификат үлгісіне арналған нұсқаулық",
        "ru": "Инструкция к шаблону сертификата",
        "en": "No t"
    }

    fund_school_link = "https://astanahub.com/ru/l/forms"

    fund_school = {
        "kz": "Қор мен мектеп арасындағы шартқа мектеп қорға барлық құжаттарды ұсынғаннан кейін қол қойылады: 1. Оқытудың толық кестесі 2. Әр студентпен үлгі шарт 3. Студенттердің дербес деректерді жинауға және өңдеуге келісімі 4. Құрылтай құжаты. Бағдарламаға қатысушы барлық қажетті құжаттарды <a href=\"{fund_school_link}\">өтінімдер</a>арнайы нысаны арқылы ұсына алады.",
        "ru": f"Договор между Фондом и Школой подписывается после того как школа предоставила все документы Фонду: 1. Подробный график обучения 2. Типовой договор с каждым студентом 3. Согласия студентов на сбор и обработку персональных данных 4. Учредительный документ. Участник Программы может предоставить все необходимые документы через специальную форму <a href=\"{fund_school_link}\">заявки</a>",
        "en": "No translate"
    }

    fund_school_template_button = {
        "kz": f"Шарт үлгісі",
        "ru": f"Шаблон договора",
        "en": f"Contract template",
    }

    fund_school_template = {
        "kz": f"<a href=\"{student_contract_link['contract_1']}\">Шарт үлгісі</a>",
        "ru": f"<a href=\"{student_contract_link['contract_1']}\">Шаблон договора</a>",
        "en": f"<a href=\"{student_contract_link['contract_1']}\">Contract template</a>",
    }

    advance_payment_message = {
        "kz": f"Осы Шартқа қол қойылғаннан және қорға барлық қажетті құжаттарды тапсырғаннан кейін Бағдарламаға қатысушының пайдасына Қор қаржыландыруға жататын қаржыландырудың жалпы сомасының 30% (отыз пайызы) мөлшеріндегі ақшалай қаражат сомасы.",
        "ru": f"Cумма денежных средств в размере 30% (тридцати процентов) от Общей суммы финансирования, подлежащая финансированию Фондом в пользу Участника Программы после подписания настоящего Договора и передачи Фонду всех необходимых документов.",
        "en": f"No translate",
    }

    get_advance_payment = {
        "kz": f"Аванс төлеу үшін не қажет?",
        "ru": f"Что нужно для того чтобы выплатили аванс?",
        "en": f"No translate",
    }

    advance_payment_link = 'https://astanahub.com/account/service/techorda_schools_before/request/266/create/'

    advance_payment_template = {
        "kz": f"Аванстық Төлем Бағдарлама Қатысушысы келесі шарттарды орындаған жағдайда төленеді: \n1. Әрбір Студенттің жеке деректерін жинау және өңдеу туралы қол қойылған келісімдерді Қорға тапсыру \n2. Қорға Қор ұсынған нысан бойынша Курстың(Курстардың) сабақ кестесін тапсыру \n3. Қорға дұрыс жасақталған төлем шотын ұсыну \n4. Мектеп пен Студент арасындағы қол қойылған барлық типтік келісім-шарттардың көшірмелерін Қорға тапсыру \n5. (Бар болған жағдайда) Бағдарлама Қатысушысы мен КИК арасындағы қол қойылған барлық келісім-шарттардың көшірмелерін Қорға тапсыру Сіз барлық жоғарыда аталған құжаттарды осы сілтеме арқылы жүктей аласыз: <a href=\"{advance_payment_link}\">ССЫЛКА</a>",
        "ru": f"Авансовый Платеж оплачивается при выполнении Участником Программы следующих условий: \n1. Передачи Фонду подписанных каждым Студентом согласий на сбор и обработку персональных данных \n2. Передачи Фонду графика занятий по Курсу(-ам) по форме, предоставленной Фондом \n3.  Предоставления Фонду надлежащим образом сформированного счета на оплату \n4. Передачи Фонду копий всех подписанных типовых договоров между Школой и Студентом \n5. (При наличии) Передачи Фонду копий всех подписанных договоров между Участником Программы и КИК. \nВы можете загрузить все вышеперечисленные документы по данной ссылке: <a href =\"{advance_payment_link}\">ССЫЛКА</a>",
        "en": f"No translate",
    }

    learning_change_message = {
        "kz": f"Қорды қандай өзгерістер туралы хабардар ету қажет: \n1. Студентті ауыстыру немесе шығару \n2. Студенттің жеке деректеріндегі өзгерістер \n3. Компанияның реквизиттеріндегі өзгерістер (Заңды мекен-жайдың өзгеруі, компания атауының өзгеруі, банк реквизиттері және басшының ауысуы) \n4. Оқу кестесіндегі өзгерістер (оқу ұзақтығы, онлайн сабаққа сілтеме, сабақ өткізу күндері мен уақыты, курс бойынша оқытушы, оқу аяқталу күні және академиялық сағаттар саны) \n5. Сабақ өткізу орны (оффлайн курстар үшін). \n\nБарлық өзгерістер astanahub.com порталында электронды түрде жүзеге асырылады.",
        "ru": f"О каких изменениях необходимо оповещать Фонд: \n1. Замена либо исключение студента \n2. Изменение в персональных данных студента \n3. Изменение реквизитов компании (Смена юридического адреса, смена названия компании, банковские реквизиты и смена руководителя) \n4. Изменения в графике обучения (длительность обучения, ссылка на онлайн урок, дни и время проведения занятий, преподаватель по курсу, дата окончания обучения и количество академ часов) \n5. Место проведения занятий (для оффлайн курсов). \n\nВсе изменения осуществляются в электронной форме на портале astanahub.com",
        "en": f"No translate",
    }

    learning_change_details = {
        "kz": f"1. Студентті ауыстыру немесе шығару \n2. Студенттің жеке деректеріндегі өзгерістер \n3. Компанияның реквизиттеріндегі өзгерістер (Заңды мекен-жайдың өзгеруі, компания атауының өзгеруі, банк реквизиттері және басшының ауысуы) \n4. Оқу кестесіндегі өзгерістер (оқу ұзақтығы, онлайн сабаққа сілтеме, сабақ өткізу күндері мен уақыты, курс бойынша оқытушы, оқу аяқталу күні және академиялық сағаттар саны) \n5. Сабақ өткізу орны (оффлайн курстар үшін)",
        "ru": f"1. Замена либо исключение студента \n2. Изменение в персональных данных студента \n3. Изменение реквизитов компании (Смена юридического адреса, смена названия компании, банковские реквизиты и смена руководителя) \n4. Изменения в графике обучения (длительность обучения, ссылка на онлайн урок, дни и время проведения занятий, преподаватель по курсу, дата окончания обучения и количество академ часов) \n5. Место проведения занятий (для оффлайн курсов)",
        "en": f"No translate",
    }

    notify_learning_change = {
        "kz": "Эл. форманы ашу?",
        "ru": "Открыть эл. форму?",
        "en": "What changes need to be reported to the Fund?",
    }

    additional_agreements = {
        "kz": "Қосымша келісімдер",
        "ru": "Дополнительные соглашения",
        "en": "No translate",
    }

    additional_agreements_message = {
        "kz": "Осы Келісімшартқа енгізілетін барлық өзгерістер Тараптардың өзара келісімі бойынша жүзеге асырылады және олар жазбаша түрде қосымша келісім түрінде жасалып, Тараптардың уәкілетті өкілдері қол қойған жағдайда жарамды болып табылады",
        "ru": "Все изменения в настоящий Договор осуществляются по взаимному согласию Сторон и действительны при условии, что они совершены в письменной форме в виде дополнительного соглашения и подписаны уполномоченными на то представителями Сторон",
        "en": "No translate",
    }

    electronic_act = {
        "kz": "Электрондық шот-фактуралар ақпараттық жүйесінде Электрондық орындалған жұмыстар актісін толтыру бойынша нұсқаулық.",
        "ru": "Памятка по заполнению Электронного акта выполненных работ в Информационной системе электронных счетов-фактур.",
        "en": "No translate",
    }

    final_payment_bill = {
        "kz": "Оқуды аяқтау бойынша соңғы төлемге шот қалыптастыру жөніндегі жадынама",
        "ru": "Памятка по формированию Счета на финальную оплату по завершению обучения",
        "en": "No t",
    }


class ITSchoolsShowcaseQuestions:
    questions = [
        {
            'question': {
                "kz": "Витринаға қандай деректер жүктеледі?",
                "ru": "Какие данные подгружаются в витрину?",
                "en": "What data is uploaded to the showcase?"
            },
            'answer': {
                "kz": "It мектептердің витринасына бағдарламаға қатысуға өтінімнен деректер тартылады",
                "ru": "В Витрину IT школ подтягиваются данные с заявки на участие в программе.",
                "en": "Data from the application for participation in the program is being pulled into the Showcase of IT schools"
            },
        },

        {
            'question': {
                "kz": "Студенттердің өтінімдер санын қайдан көруге болады?",
                "ru": "Где можно посмотреть количество заявок от студентов?",
                "en": "Where can I see the number of applications from students?"
            },
            'answer': {
                "kz": "astanahub.com порталында, \"Tech Orda\" және \"Менің өтініштерім\" бөліміне өтіңіз",
                "ru": "На портале astanahub.com, заходите в раздел \"Tech Orda\" и в \"Мои заявки\"",
                "en": "On the portal astanahub.com , go to the \"Tech Orda\" section and to \"My applications\""
            },
        },

        {
            'question': {
                "kz": "Менің деректерім витринаға дұрыс енгізілмеген бе?",
                "ru": "У меня некорректно сели данные в Витрину?",
                "en": "Is my data entered incorrectly in the showcase?"
            },
            'answer': {
                "kz": "Мәселеңізді егжей тегжейлі сипаттаңыз",
                "ru": "Развернуто опишите вашу проблему",
                "en": "Describe your problem in detail"
            },
        },

    ]


class MonthlyReportQuestions:
    questions = [
        {
            'question': {
                "kz": "Ай сайынғы есепті толтыру жөніндегі Нұсқаулық",
                "ru": "Инструкция по заполнению ежемесячного отчета",
                "en": "No translation"
            },
            'answer': {
                "kz": "инструкция_отчетности_для_школы_2024.docx",
                "ru": "инструкция_отчетности_для_школы_2024.docx",
                "en": "инструкция_отчетности_для_школы_2024.docx"
            },
        },

        {
            'question': {
                "kz": "Студенттердің есепті толтыру жөніндегі нұсқаулығы",
                "ru": "Инструкция по заполнению отчета студентами",
                "en": "No translation"
            },
            'answer': {
                "kz": "link",
                "ru": "форма_подтверждения_отчета_(для студентов).docx",
                "en": "link"
            },
        },

    ]


class CompetenceAssessmentQuestions:
    questions = [
        {
            'question': {
                "kz": "Тестілеуден өту үшін қанша ұпай жинау керек",
                "ru": "Сколько нужно набрать баллов для прохождения тестирования",
                "en": "No t"
            },
            'answer': {
                "kz": "Біліктілікті бағалау шеңберінде оқудан өткен және өту балын - 50% (елу пайыз) жинаған Студент оқуды аяқтаған студенттер деп танылады.",
                "ru": "Завершившим обучение Студентом признается Студент, прошедший обучение и набравший проходной балл - 50% (пятьдесят процентов) в рамках Оценки компетенций.",
                "en": "No t"
            },
        },

        {
            'question': {
                "kz": "Әр студентке тестілеу қажет пе?",
                "ru": "Обязательно ли сдавать тестирование каждому студенту?",
                "en": "No t"
            },
            'answer': {
                "kz": "Студент адалдық және Академиялық этика қағидаларын сақтай отырып, курс бойынша оқу барысында құзыреттілікті бағалау шеңберінде тестілеуге және(немесе) өзге де іс-шараларға қатысуға міндеттенеді.",
                "ru": "Студент обязуется принять участие в тестировании и(или) иных мероприятиях в рамках Оценки компетенций в течение обучения по Курсу, следуя правилам честности и академической этики.",
                "en": "No t"
            },
        },

        {
            'question': {
                "kz": "Мен өз сұрақтарымды тестілеуге қосқым келеді",
                "ru": "Я хочу добавить свои вопросы в тестирование",
                "en": "No t"
            },
            'answer': {
                "kz": "Сіз өзіңіздің сұрақтарыңызды берілген <a href=\"бойынша тестілеуге қоса аласыз https://astanahub.com/account/service/techorda_questions/request/253/create\" >сілтеме</a>",
                "ru": "Вы можете добавить ваши вопросы в тестирование по данной <a href=\"https://astanahub.com/account/service/techorda_questions/request/253/create/\">ССЫЛКЕ</a>",
                "en": "No t"
            },
        },

        {
            'question': {
                "kz": "Тестілеуден өтуге арналған нұсқаулар",
                "ru": "Инструкции для прохождения тестирования",
                "en": "No t"
            },
            'answer': {
                "kz": "no t",
                "ru": "no t",
                "en": "no t"
            },
        },

    ]
